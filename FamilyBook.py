#
# Gramps - a GTK+/GNOME based genealogy program - Family Book plugin
#
...
class FamilyBook(Report):
    """
    A Family Book is a page which contains all available info about a specific
    ...
    Each Family Book contains a key at the top right which is derived from the
    relationship from the central person in the database and the person for
    which the sheet is printed. For direct ascendants, the Family Bool key is
    ...
    Family Book key of the child is the Family Book key of the parent with
    ...
    and birth event info) is printed along with a reference to the Family Book
    page on which this child would be the key person, while for children that
    had no spouses, full info is printed.

    If recursive printing is selected, each Family Book is followed by the
    Family Books of the children that had spouses.
    """
    ...
    def __init__(self, database, options, user):
        ...
        self.recurse      = FamilyBookOptions.RECURSE_NONE
        self.callname     = _Name_CALLNAME_DONTUSE
        self.placeholder  = True
        self.incl_sources = True
        self.incl_notes   = True
    ...
    def __process_person(self, person, rank, ahnentafel, person_key):
        """
        Recursively build the Family Book for this person and all children
        with spouses.

        @param person: Person object for the key person of the Family Book.
        ...
        @param person_key: Family Book key to be printed on the top right of
            the corner.
        """
        ...
        self.doc.write_text(self.__format_name(person))
        self.doc.write_text('}')
        self.doc.end_paragraph()
        #!!!
...
#                spouse = None
            ...
            # person, we refer to the Family Book of the spouse instead of
...
#                self.__process_person(child, child_rank, child_at, child_key)
    ...
    def __dump_person(self, person, short, ref):
        ...
        @param ref: Reference through which this person is linked into the
            Family Book. Can be a family object (for the spouses) or a
